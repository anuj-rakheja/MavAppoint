...
public class CheckUser extends SQLCmd{
	...
	public void queryDB(){
		try{
			String command = "SELECT COUNT(*),ROLE,pname FROM USER,advisor_settings WHERE user.userid=advisor_settings.userid AND user.EMAIL=? AND user.PASSWORD=? AND user.validated=1";
			...
			command = "SELECT COUNT(*),ROLE,pname FROM USER,advisor_settings WHERE user.EMAIL=? AND user.PASSWORD=? AND user.VALIDATED=1";
